...
<div v-for="(p,indexes) in productos" v-bind:key="indexes" class="p-5">
<div class="card" style="width: 18rem;">
  <img class="card-img-top" :src=p.Img alt="Card image cap" style="max-height:200px" >
  <div class="card-body">
    <h5 class="card-title"><strong>{{p.name}}</strong></h5>
    <div class="clearfix">
      <div class="precio" style="float:left"> L.{{p.unitPrice}}</div>
    <a  class="btn btn-success pull-right" @click="agregar(p._id)" style="float:right" role="button">Agregar al carrito</a>
...
export default {
  ...
  methods: {
    mostrarP() {
      axios
        .get(
          "http://localhost:3000/customer/" + this.$route.params.page
        )
        .then(res => {
  ...
  computed: {
    ...
    indices: function() {
      var indices=[];
      var indice = this.ind;
      for (indice;(indice <= (Number(this.current) + 4 ))&&( indice <= this.pages);indice++ ) {
